...
class TraceContext {
  ...
  static startOrResume(childOf, conf, tracestateArg, sampleRate) {
    if (childOf && childOf._context instanceof TraceContext)
      return childOf._context.child();
    const traceparent = TraceParent.startOrResume(childOf, conf, sampleRate);
    const tracestate =
    ...
    const effectiveRate =
      typeof sampleRate === 'number' ? sampleRate : conf.transactionSampleRate;

    if (!childOf && traceparent.recorded) {
      // if this is a sampled/recorded transaction, set the rate
      tracestate.setValue('s', effectiveRate);
    } else if (!childOf) {
