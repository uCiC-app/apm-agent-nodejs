...
//    - opts.sampleRate - A number from 0 - 1 that overrides the global
//      `transactionSampleRate` config for this transaction. Only applies to
//      root transactions (i.e. when the trace is not continued from a parent).
...
function Transaction(agent, name, ...args) {
  ...
  // Validate and normalize the per-transaction sampleRate option.
  if (opts.sampleRate !== undefined) {
    if (
      typeof opts.sampleRate !== 'number' ||
      opts.sampleRate < 0 ||
      opts.sampleRate > 1
    ) {
      agent.logger.warn(
        'Invalid sampleRate option %s, must be a number between 0 and 1 -- ignoring',
        opts.sampleRate,
      );
      opts.sampleRate = undefined;
    } else {
      opts.sampleRate = Math.round(opts.sampleRate * 1e4) / 1e4;
    }
  }
